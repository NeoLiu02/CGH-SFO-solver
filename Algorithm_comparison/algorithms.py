...
# 1. GS
def gerchberg_saxton(init_phase, target_amp, num_iters, prop_dist, wavelength, feature_size=3.74e-6,
                     phase_path=None, prop_model='ASM', propagator=None,
                     writer=None, dtype=torch.float32, precomputed_H_f=None, precomputed_H_b=None):
    """
    Given the initial guess, run the GS algorithm to calculate the optimal phase pattern of spatial light modulator
    ...
    :return: a tensor, the optimized phase pattern at the SLM plane, in the shape of (1,1,H,W)
             a tensor, the final reconstruction intensity at the sensor plane, in the shape of (1,1,H,W)
    """
    ...
    out_inten_scaled = out_inten * torch.sum(target_inten, dim=(-2, -1), keepdim=True)\
                       / torch.sum(out_inten, dim=(-2, -1), keepdim=True)

    # return phases
    return slm_field.angle(), out_inten_scaled
...
def stochastic_gradient_descent(init_phase, target_amp, num_iters, prop_dist, wavelength, feature_size,
    ...
    :param prop_dist: propagation distance in m.
    :param wavelength: wavelength in m.
    :param feature_size: the SLM pixel pitch, in meters, default 3.74e-6
    ...
    return slm_phase, out_inten_scaled
...
def double_phase_amplitude_coding(target_phase, target_amp, prop_dist, wavelength, feature_size,
    ...
    end_time = time.time()
    print("DPAC runtime: ", end_time-start_time)
