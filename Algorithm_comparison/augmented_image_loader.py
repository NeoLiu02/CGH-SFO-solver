...
class ImageLoader:
    ...
    def __init__(self, data_path, channel=None, batch_size=1,
                 image_res=(2160, 3840), homography_res=(2160, 3840),
    ...
    def load_image(self, filenum, *augmentation_states):
        ...
        im = simple_resize(im, self.image_res)
...
def simple_resize(image, target_res):
    """Simply Resize image to the target_res
    ...
    image = np.transpose(image, axes=(1, 2, 0))
    shape = image.shape
    image = cv2.resize(image, (target_res[1], target_res[0]), interpolation=cv2.INTER_LINEAR)
    image = image.reshape(target_res[0], target_res[1], shape[2])
    return np.transpose(image, axes=(2, 0, 1))
