...
def propagation_ASM(u_in, feature_size, wavelength, z, linear_conv=True,
                    ...
                    dtype=torch.float32, apply_constraint=True):
    """Propagates the input field using the angular spectrum method with band constraint
    ...
    if precomped_H is None:
        ...
        if apply_constraint:
            fxlimit = 1 / 3 / feature_size[0]
            fylimit = 1 / 3 / feature_size[1]
            H_filter2 = torch.tensor(((np.abs(FX) < fxlimit) & (np.abs(FY) < fylimit)).astype(np.uint8), dtype=dtype)
            H_filter = H_filter*H_filter2
