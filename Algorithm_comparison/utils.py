...
def propagate_field(input_field, propagator, prop_dist=0.1, wavelength=520e-9, feature_size=(3.74e-6, 3.74e-6),
...
def write_sgd_summary(slm_phase, out_amp, target_amp, k,
    ...
    if writer is not None:
        ...
        loss_value = loss(recon, target)
        psnr_value = psnr(target.squeeze().cpu().detach().numpy(), recon.squeeze().cpu().detach().numpy())
        ssim_value = ssim(target.squeeze().cpu().detach().numpy(), recon.squeeze().cpu().detach().numpy())

        recon0 = out_amp ** 2
        scale = torch.sum(target, dim=(-2, -1), keepdim=True) / torch.sum(recon0, dim=(-2, -1), keepdim=True)
        psnr_value_scale = psnr(target.squeeze().cpu().detach().numpy(), (scale * recon0).squeeze().cpu().detach().numpy())
        ssim_value_scale = ssim(target.squeeze().cpu().detach().numpy(), (scale * recon0).squeeze().cpu().detach().numpy())

        writer.add_image(f'{prefix}_Recon/inten', recon.squeeze(0), k)
        ...
        writer.add_scalar(f'{prefix}_psnr/scaled', psnr_value_scale, k)
        writer.add_scalar(f'{prefix}_ssim/scaled', ssim_value_scale, k)

        writer.add_scalar(f'{prefix}_optimized_scalar', s, k)
        writer.add_scalar(f'{prefix}_scale', scale, k)


def write_gs_summary(slm_field, recon_field, target_amp, k, writer, prefix='test'):
    ...
    if writer is not None:
        ...
        recon *= (torch.sum(target, (-2, -1), keepdim=True)
                      / torch.sum(recon, (-2, -1), keepdim=True))

        loss_value = loss(recon, target)
        psnr_value = psnr(target.squeeze().cpu().detach().numpy(), recon.squeeze().cpu().detach().numpy())
        ssim_value = ssim(target.squeeze().cpu().detach().numpy(), recon.squeeze().cpu().detach().numpy())

        writer.add_image(f'{prefix}_Recon/amp', recon.squeeze(0), k)
        writer.add_scalar(f'{prefix}_loss', loss_value, k)
        writer.add_scalar(f'{prefix}_psnr', psnr_value, k)
        writer.add_scalar(f'{prefix}_ssim', ssim_value, k)
